...
function CardListPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [sortBy, setSortBy] = useState<
    "intervalStrength" | "nextRepetitionTime" | "createdAt" | undefined
  >("intervalStrength");
  ...
  if (isLoading)
    return (
      <div className={styles.cardListFallback}>
        <Spinner size={60} />
      </div>
    );
  ...
  if (error) {
    console.log(error);
    return (
      <div className={styles.cardListFallback}>Failed to load this deck</div>
    );
  }
  if (!deck || !cards)
    return <div className={styles.cardListFallback}>No deck found</div>;
  ...
  return (
    <>
      ...
      <CardListFooter
        ...
        cardsCount={cards.items.length}
        onDelete={() => setDeleteOpen(true)}
        onMove={() => setMoveCardOpen(true)}
